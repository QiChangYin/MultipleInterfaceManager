...
from HttpApiManager.models import ModuleInfo, TestCaseInfo, TestReports
from HttpApiManager.utils.operation import add_project_data, add_module_data, add_case_data, add_config_data, \
    add_register_data
...
def task_logic(**kwargs):
    ...
    if 'module' in kwargs.keys():
        kwargs.pop('project')
        return create_task(name, 'HttpApiManager.tasks.module_hrun', kwargs, crontab, desc)
    else:
        return create_task(name, 'HttpApiManager.tasks.project_hrun', kwargs, crontab, desc)
